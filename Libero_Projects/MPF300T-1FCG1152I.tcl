...
set Libero_project_name_axi PF_EVAL_KIT_MiV_AXI_BaseDesign

set project_folder_name_ahb MiV_AHB_BD
set project_dir_ahb "./$project_folder_name_ahb"
set Libero_project_name_ahb PF_EVAL_KIT_MiV_AHB_BaseDesign
...
proc create_new_project_label_axi { }\
{
   ...
   puts "------------PF_EVAL_KIT_MiV_AXI_BaseDesign--------------------------"
   puts "-----------------------------------------------------------------------------------------------"
   puts "-----------------------------------------------------------------------------------------------"
}
...
proc create_new_project_label_ahb { }\
{
   ...
   puts "-----------------------------------------------------------------------------------------------"
   puts "---------------------------Creating New Project------------------------------------"
   puts "------------PF_EVAL_KIT_MiV_AHB_BaseDesign------------------------"
   puts "-----------------------------------------------------------------------------------------------"
   puts "-----------------------------------------------------------------------------------------------"
}
...
if {"$target" == "AXI"} then {
	if {[file exists $project_dir_axi] == 1} then {
		...
		source ./import/components/PF_Eval/AXI/import_component_and_constraints_pf_eval_axi.tcl
		save_project
        base_design_built
	}
	...
	if {[file exists $project_dir_ahb] == 1} then {
		...
		source ./import/components/PF_Eval/AHB/import_component_and_constraints_pf_eval_ahb.tcl
		save_project
        base_design_built
	}
...
if {"$design_flow_stage" == "SYNTHESIZE"} then {
	...
	if {"$target" == "AHB"} then {
		export_prog_job \
			-job_file_name {PF_EVAL_KIT_MiV_AHB_BaseDesign} \
		...
		export_prog_job \
			-job_file_name {PF_EVAL_KIT_MiV_AXI_BaseDesign} \
