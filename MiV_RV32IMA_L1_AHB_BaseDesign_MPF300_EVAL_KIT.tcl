...
append Libero_project_name "_MPF300_EVAL_KIT"
append project_dir "_MPF300_EVAL_KIT"
append project_folder_name "_MPF300_EVAL_KIT"
...
proc create_new_project_label { }\
{
	...
	puts "----------MiV_RV32IMA_L1_AHB_BaseDesign_MPF300_EVAL_KIT------------------"
    puts "-------------------------------------------------------------------------"
}
...
if {[file exists $project_dir] == 1} then { 
	...
	new_project -location $project_dir -name $Libero_project_name -project_description {} -block_mode 0 -standalone_peripheral_initialization 0 -instantiate_in_smartdesign 1 -ondemand_build_dh 1 -hdl {VERILOG} -family {PolarFire} -die {MPF300T} -package {FCG1152} -speed {-1} -die_voltage {1.0} -part_range {IND} -adv_options {IO_DEFT_STD:LVCMOS 1.8V} -adv_options {RESTRICTPROBEPINS:1} -adv_options {RESTRICTSPIPINS:0} -adv_options {SYSTEM_CONTROLLER_SUSPEND_MODE:0} -adv_options {TEMPR:IND} -adv_options {VCCI_1.2_VOLTR:EXT} -adv_options {VCCI_1.5_VOLTR:EXT} -adv_options {VCCI_1.8_VOLTR:EXT} -adv_options {VCCI_2.5_VOLTR:EXT} -adv_options {VCCI_3.3_VOLTR:EXT} -adv_options {VOLTR:IND} 
	source ./MPF300_EVAL_KIT/import_component_and_constraints_mpf300_eval_kit.tcl
	save_project
}
...
if {"$design_flow_stage" == "SYNTHESIZE"} then {
	...
	run_tool -name {VERIFYTIMING}
    export_prog_job \
        -job_file_name {BaseDesign_MPF300_EVAL_KIT-ProgramFile} \
        -export_dir {./} \
